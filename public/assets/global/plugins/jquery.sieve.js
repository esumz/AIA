...
(function() {
    ...
    $.fn.sieve = function(options) {
        ...
        return this.each(function() {
            var container, searchBar, settings, countItem;
            container = $(this);
            countItem = 0;
            settings = $.extend({
                searchLimit: 5,
            ...
            return settings.searchInput.on("keyup.sieve change.sieve", function() {
                var items, temp, query, count = 0;
                temp = $.trim($(this).val());
                query = compact(temp.toLowerCase().split(/\s+/));
                ...
                items.each(function() {
                    ...
                    match = count > settings.searchLimit ? false : match;
